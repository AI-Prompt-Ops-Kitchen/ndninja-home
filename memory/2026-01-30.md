...
✅ Full integration test passed:
   - TTS generation → CapCut audio track
   - Video → CapCut video track (with looping)
   - Saves draft successfully

### Usage
```bash
# Start CapCut server (background)
cd /home/ndninja/projects/capcut-api && source venv/bin/activate && python capcut_server.py &

# Generate content with CapCut output
python scripts/ninja_content.py --script "Your script" --capcut --no-music

# With B-roll
python scripts/ninja_content.py --auto --capcut --broll --no-music
```

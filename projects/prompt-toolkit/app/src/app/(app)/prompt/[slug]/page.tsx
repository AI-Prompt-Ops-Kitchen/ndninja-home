...
const MOCK_PROMPT = {
  ...
  description:
    'Get expert-level code review feedback with actionable suggestions for improvement, security concerns, and best practices. Works with any programming language.',
  content: `You are a senior software engineer with 15+ years of experience conducting code reviews. Review the following code with these priorities:
...
- Suggest concrete improvements (show the better code)
...
{{code}}
\`\`\``,
  system_prompt: 'You are an expert code reviewer. Be thorough but constructive. Always explain WHY something should change, not just what.',
  example_output: `## Code Review: UserService.ts
...
### 🟡 Important
**Line 45:** Missing error handling for the API call — if this fails, the entire request silently returns undefined.

### 🟢 Nice-to-have
**Line 12:** Consider extracting the magic number 30 into a named constant.

### Overall: 6/10
Solid logic but needs security fixes before merge. The architecture is clean — just needs hardening.`,
  use_case: 'Use before submitting PRs, during pair programming, or when reviewing unfamiliar codebases.',
  variables: [
    { name: 'language', description: 'Programming language of the code', placeholder: 'typescript' },
    { name: 'code', description: 'The code to review', placeholder: 'function fetchUser(id) { ... }' },
  ],
  difficulty: 'advanced',
  category: 'Code & Development',
  ai_model_tags: ['GPT-4', 'Claude', 'Gemini'],
  author: { display_name: 'PromptKit Team', avatar: 'PK' },
  copy_count: 1247,
  favorite_count: 89,
  rating_avg: 4.8,
  rating_count: 34,
...
const DNA_COMPONENTS = [
  { label: 'Role Definition', color: 'bg-indigo-500/20 text-indigo-400 border-indigo-500/30', desc: 'Sets the AI as a senior engineer' },
  { label: 'Task Framing', color: 'bg-purple-500/20 text-purple-400 border-purple-500/30', desc: 'Structured checklist approach' },
  { label: 'Constraints', color: 'bg-cyan-500/20 text-cyan-400 border-cyan-500/30', desc: 'Specific, actionable feedback' },
  { label: 'Output Format', color: 'bg-green-500/20 text-green-400 border-green-500/30', desc: 'Severity-rated findings' },
  { label: 'Variables', color: 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30', desc: 'Language and code inputs' },
];

const RELATED = [
  { title: 'React Component Generator', slug: 'ui-component-generator', category: 'Code & Development', rating: 4.8 },
  { title: 'API Documentation Writer', slug: 'api-doc-writer', category: 'Code & Development', rating: 4.6 },
  { title: 'Unit Test Generator', slug: 'unit-test-gen', category: 'Code & Development', rating: 4.7 },
];
...
export default function PromptDetailPage({
  ...
  const filledPrompt = prompt.content.replace(/\{\{(\w+)\}\}/g, (_, key) =>
            ...
            <Card>
              <CardContent className="p-5">
                ...
                <div className="space-y-3">
                  {DNA_COMPONENTS.map((component, i) => (
                    <motion.div
                      key={component.label}
                    ...
                    >
                      <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium border ${component.color}`}>
                        {component.label}
                      </span>
                      <span className="text-sm text-gray-400">{component.desc}</span>
                    </motion.div>
                  ))}
                </div>
              </CardContent>

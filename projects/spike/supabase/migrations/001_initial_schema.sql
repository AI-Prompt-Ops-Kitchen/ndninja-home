-- Spike Mood Tracker — Initial Schema
...
-- spikes: users can only CRUD their own
create policy "Users can view own spikes"
  on spikes for select using (auth.uid() = user_id);
create policy "Users can insert own spikes"
  on spikes for insert with check (auth.uid() = user_id);
create policy "Users can update own spikes"
  on spikes for update using (auth.uid() = user_id);
create policy "Users can delete own spikes"
  on spikes for delete using (auth.uid() = user_id);

-- timer_sessions: users can only CRUD their own
create policy "Users can view own timer sessions"
  on timer_sessions for select using (auth.uid() = user_id);
create policy "Users can insert own timer sessions"
  on timer_sessions for insert with check (auth.uid() = user_id);
create policy "Users can update own timer sessions"
  on timer_sessions for update using (auth.uid() = user_id);

-- email_log: users can only view their own
...
from spikes
group by user_id, extract(hour from logged_at), type;
...
select
  ...
  count(*) as spike_count
from spikes
group by user_id, extract(dow from logged_at), extract(hour from logged_at);
